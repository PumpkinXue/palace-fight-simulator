...
一步生成所有选项的结果，不再需要第二次调用。

地点说明：
- 御书房：皇帝读书习字、处理私密政务之地，氛围更静谧庄重，核心关联维度：皇帝好感度、玩家才情/心机、是否通文墨，态度判定标准与养心殿一致。
触发条件细分：
（1）低阶触发（刚入宫/位份≤从四品/圣宠≤40）：被内侍拦下，告知皇帝“不喜打扰”，仅能留下物品代为转达；若坚持入内，触发皇帝厌烦，降圣宠及才情评价。
（2）中阶触发（位份＞从四品/圣宠41-80）：内侍通报后可入内等候，需保持安静；皇帝可能询问读书心得，或让玩家磨墨、整理书籍；若通文墨，可短暂探讨简单典籍。
（3）高阶触发（圣宠＞80/位份≥正三品/才情＞50）：皇帝主动邀请共研诗书，允许翻阅珍藏典籍；可能让玩家品鉴书法、代笔誊写文书；涉及私密政务时，可能让玩家回避或旁听。
核心事件场景：
...
触发条件细分：
（1）新手触发（入宫＜3个月/位份≤答应）：需严格遵循礼仪，由内侍引导请安；大概率仅能得到太后简短回应，重点考察跪拜礼仪、言行举止。
（2）常规触发（入宫≥3个月/位份≥常在）：太后会询问饮食起居、后宫近况；若近期有善事（如体恤宫人、进献养生物品），会获得认可；若有失仪行为，可能被训诫。
（3）特殊触发（位份≥妃位/太后好感＞60/有子嗣）：太后可能与玩家闲聊家事、探讨育儿之道；委托处理小型后宫事务（如查验祭祀物品、教导低位份妃嫔礼仪）；在皇帝面前为玩家美言。
核心事件场景：
...
触发条件细分：
（1）友好触发（好感度＞50）
目标妃嫔主动上前迎接玩家，行为符合其性格设定：温婉型分享后宫趣闻、倾诉内心烦闷；聪慧型提出结盟请求（如联手制衡敌对势力、互相引荐争宠）；大方型赠送玩家增益道具（如养颜膏提升容貌、才艺典籍提升琴棋书画属性）；娇羞型悄悄谈论对皇上的心意与期待；体贴型关怀玩家近况、提醒宫廷潜在风险。
（2）中立触发（好感度 10-50）
目标妃嫔偶遇玩家后礼貌寒暄，不冷不热保持距离：谈论无关紧要的话题（如天气阴晴、御花园花卉长势、即将举办的宫廷宴会流程）；不主动透露自身处境与计划，也不打探玩家隐私；互动点到即止，无明显好感增减或利益牵扯。
（3）敌对触发（好感度＜10）
目标妃嫔借故刁难玩家，行为契合其性格：跋扈型故意让玩家在宫道久立等候、命宫人端上劣质茶点羞辱；阴险型设下小陷阱（如污蔑玩家碰掉其发簪玉佩、暗中让宫人在茶水中加少量泻药）；嘴碎型当着路过的宫人、小太监的面诋毁玩家名声（如说玩家恃宠而骄、攀附高位）。
核心事件场景：
（1）日常社交场景
...
触发条件细分：
（1）机遇触发（无随从/低好感度但运气值高/特定时间）：偶遇独自散心的皇帝（提升好感度的绝佳机会）；遇到精通才艺的宫人/太妃，获得才艺指点；发现珍稀花卉/信物（可作为礼物进献）。
（2）常规触发：遇到其他妃嫔带着宫人散步，需选择主动打招呼、绕道回避、驻足闲聊；欣赏风景时遇到宫人打理花草，可选择帮忙（提升口碑）、旁观。
（3）危机触发（敌对妃嫔在场/无随从/夜晚独行）：被敌对妃嫔故意冲撞、污蔑破坏花草；遇到宫人暗中使绊（如脚下放石子、泼水）；被诬陷与外男私会（需及时自证）。
核心事件场景：
...
特别注意：
1. 所有变动值必须是数字，正数表示增加，负数表示减少
2. 必须包含所有人物和所有属性，即使没有变动也要显示为0
3. 事件描述要体现地点特色和人物性格
4. 考虑近期事件对当前事件的影响
5. 符合古代宫廷逻辑规矩和上下文人物关系逻辑。注意，妃嫔与玩家之间归根到底是利益冲突的，因此玩家得宠或者博得皇上宠爱，大概率会遭受其他妃嫔嫉妒，没有绝对的友好和友情。文字风格，简单易懂，不使用繁杂语句，尽量贴合现代人理解思维，语句不跳脱，剧情不跳脱，剧情发展要缓慢连贯。
6. 所有事件都必须与输入信息中的人物、事件、地点、时间等信息相关联，不可凭空出现。
7. 同一地点不要突然出现很多人物，如玩家进入尚宫局，不要写“梅妃在与女官探讨琵琶技巧，而婉嫔在一旁静静地练习书法“，这是不合理且虚假的，不符合古代妃嫔真实的生活场景。就算有很多人物，也要有这些人物出现在这里的原因，如近期事件中出现了宴会准备等等。
8. 每个选项下生成的好感度变动不要涉及与本次事件无关的人，如需涉及，也要说明那人为什么会受到影响。如玩家进养心殿只有玩家和皇帝二人，最后却加减了一堆妃嫔的好感度，这是不合理的。
...
3. 赏赐或处罚同样需要结合事件簿来看，但近期已出现的赏赐和处罚不要重复出现。例如近期已有【王贵人（替换为实际妃嫔）因嫉恨产生言行不当，禁足一月】的处罚，则不要重复出现。
4. 当妃嫔满足以下任一条件时，可能被打入冷宫或处死：
    1. 近期犯下重大过错（如谋害皇嗣、欺君罔上）
    2. 连续3个月圣宠低于10且权势低于20
    3. 犯上作乱、结党营私被揭发
    4. 家族获罪株连
    5. 失德失仪，败坏宫规
5. 消亡类型
...
7. 若好感度变动和属性变动中有本次事件中没有直接参与的人物，需要在描述中说清楚原因，禁止在输出结果中突然出现事件之外的人物。如玩家和妃嫔A交互，却扣减了妃嫔B的好感度，需要说明“此事传到了妃嫔B那里……”“妃嫔A将此事告知了B……”等等这样的叙述。但仅有0.3的概率传出去。
...
输入信息包括：
1. 情况分类：九种情况之一
2. 事件模板：你需要根据这个模版和具体情况改写（根据妃嫔的具体情况），替换其中的姓名、位分（如玩家位分姓名、具体妃嫔姓名等）为用户输入的实际信息，并不要出现任何不正确的姓名和信息。若模版为某妃嫔嘲讽玩家，则需要根据妃嫔对玩家的实际好感度来改写（好感度尚可则不应嘲讽），反之同理。
3. 玩家信息：姓名、身份、位分、各项属性
4. 皇帝信息：姓名、性格、介绍
5. 后宫妃嫔信息
6. 好感度信息
7. 近期事件簿
...
九种情况分类：
1. 低好感低位分（好感<40，位分等级>=10）
2. 低好感中位分（好感<40，位分等级>=4且<10）
3. 低好感高位分（好感<40，位分等级<4）
4. 中好感低位分（40<=好感<80，位分等级>=10）
5. 中好感中位分（40<=好感<80，位分等级>=4且<10）
6. 中好感高位分（40<=好感<80，位分等级<4）
7. 高好感低位分（好感>=80，位分等级>=10）
8. 高好感中位分（好感>=80，位分等级>=4且<10）
9. 高好感高位分（好感>=80，位分等级<4）
...
你的任务：
1. 根据提供的模板，替换其中的姓名、位分（如玩家位分姓名、具体妃嫔姓名等）为实际游戏信息，并根据妃嫔的擅长才艺、性格以及对玩家的好感度还有实际位分合理生成剧情。
2. 改写模板为200-300字的详细事件描述，保持原有文字风格。你需要根据近期事件和人物关系联想，这个模版只是给你提出了一个大方向，但不允许在模版中没有其余妃嫔的情况下出现其余妃嫔在场。
3. 生成3个选项，并为每个选项生成选择后的结果，包括：
   - 结果描述：100-150字（这个结果描述只需要停留在玩家进入养心殿之前即可，因为玩家获得许可后还需要选择下一步的互动，你只是模拟求见事件的演进。）
   - 是否获得进入养心殿的许可（布尔值）
   - 玩家属性变动
   - 人物好感度变动
   - 妃嫔属性变动（如果有相关妃嫔）
...
输出格式：严格的JSON格式
{
    ...
    "选项": [
        ...
        },
        {
            "选项描述": "借机吸引",
            "结果描述": "...",
            "获得进入许可": true,  // 注意：在低宠爱情况下，只有少数情况下可能获得许可（近期印象较好、皇帝心情较好或有高影响力妃嫔美言），中宠爱则视情况而定，高宠爱获得许可的几率较高
            "玩家属性变动": {...},
            "人物好感度变动": {...},
            "妃嫔属性变动": [...],
            "事件概括": "..."
...
生成要求：
1. 根据太医当前状态（是否有主子），生成合理的事件描述（200-300字）
2. 如果太医已有主子：
   - 太医会婉拒或转移话题
   - 太医可能将此事告知其主子
   - 玩家和太医的好感度可能降低
...
def generate_heartroom_event():
    ...
    try:
        ...
        print(f"无法连接到DeepSeek API: {e}")
        return jsonify({"success": False, "error": "无法连接到AI服务，请检查网络或API配置"})
    except Exception as e:
        print(f"DeepSeek API调用异常: {e}")
        return jsonify({"success": False, "error": f"AI服务调用失败: {str(e)}"})
